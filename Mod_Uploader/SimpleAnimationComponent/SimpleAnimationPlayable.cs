...
public partial class SimpleAnimationPlayable : PlayableBehaviour
{
    ...
    Dictionary<string, int> _statesIndex = new Dictionary<string, int>();
    ...
    {
        if (_statesIndex.ContainsKey(name))
        {
            return _statesIndex[name];
        }
        ...
        _statesIndex.Add(name, state.index);
    ...
    public bool PlayQueued(string name, QueueMode queueMode)
    {
        ...
        return PlayQueued(index, queueMode);
    }
    ...
    {
        return m_States.TimeRate(i);
    }

    /// <summary>現在の再生時間</summary>
    public float CurTime(int i = 0)
    {
        return m_States.CurTime(i);
    }

    /// <summary>最大再生時間</summary>
    public float MaxTime(int i = 0)
    {
        return m_States.MaxTime(i);
    }

    public bool IsLoop(int i = 0)
    {
        var wrapMode = m_States.GetStateWrapMode(i);
        return wrapMode == WrapMode.Loop || (wrapMode == WrapMode.Default && m_States.GetStateClip(i).isLooping);
    }


    public bool IsDone()
    {
        return m_States.isDone();
    }
    ...
    public int FirstPlayingIndex()
    {
        return m_States.FirstPlayingIndex();
    }
    ...
    public bool CrossfadeQueued(string name, float time, QueueMode queueMode)
    {
        ...
        return CrossfadeQueued(index, time, queueMode);
    }
    ...
    public bool Blend(string name, float targetWeight, float time)
    {
        ...
        return Blend(index, targetWeight, time);
    }
    ...
    private void UpdateStates(float deltaTime)
    {
        ...
        for (int i = 0; i < m_States.Count; i++)
        {
            ...
            // if (state.enabled && (state.wrapMode == WrapMode.Default || state.wrapMode == WrapMode.Once))
