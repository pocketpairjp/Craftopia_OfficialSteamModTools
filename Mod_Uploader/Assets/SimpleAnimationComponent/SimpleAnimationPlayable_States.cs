...
public partial class SimpleAnimationPlayable : PlayableBehaviour
{
    ...
    private class StateManagement
    {
        ...
        {
            return CurTime(i) / MaxTime(i);
        }

        /// <summary>現在の再生時間</summary>
        public float CurTime(int i = 0)
        {
            return (float)m_States[i].playable.GetTime();
        }

        /// <summary>最大再生時間</summary>
        public float MaxTime(int i = 0)
        {
            return m_States[i].clip.length;
        }


        public bool isDone(int i = 0)
        {
            return m_States[i].enabled;
        }

        public bool AnyStatePlaying()
        {
            return FirstPlayingIndex() != -1;
        }

        public int FirstPlayingIndex()
        {
            return m_States.FindIndex(s => s != null && s.enabled);
        }
